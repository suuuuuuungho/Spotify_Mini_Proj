...
export default function Search() {
  ...
  const composer = (
    <form onSubmit={send} className={`shrink-0 px-lg pt-md ${hasMessages ? "pb-xl" : "pb-md"}`}>
      <div className="relative max-w-2xl mx-auto">
        <div className="relative overflow-hidden flex items-center gap-sm rounded-full p-1 backdrop-blur-xl bg-surface-container-highest/0 border border-t-white/40 border-x-white/10 border-b-black/30 shadow-[0_8px_28px_rgba(0,0,0,0.35),inset_0_1px_0_rgba(255,255,255,0.3),inset_0_-6px_10px_-8px_rgba(0,0,0,0.5)]">
          <div className="pointer-events-none absolute -top-3 left-8 w-20 h-6 rounded-full bg-white/40 blur-lg" />
          <input
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Ask for a genre, artist, or vibe..."
            autoFocus
            className="relative flex-1 rounded-full bg-transparent border-0 py-sm px-md text-on-surface focus:outline-none font-body-lg placeholder:text-on-surface-variant/50"
          />
          <button
            disabled={loading || !input.trim()}
            className="relative w-10 h-10 shrink-0 rounded-full bg-primary text-on-primary flex items-center justify-center hover:scale-105 transition-transform disabled:opacity-40 disabled:hover:scale-100"
          >
  ...
  return (
    ...
    >
      {!hasMessages && (
        <h1 className="font-vibe text-[40px] font-black tracking-[-0.02em] text-on-surface text-center mb-lg px-lg">
